...
async function sendMail(details) {
  ...
  const emailTemplate = fs.readFileSync('utils/' + details.emailFile, 'utf8');
    const transporter = nodemailer.createTransport({
        ...
        auth: {
          user: process.env.NODE_ENV === 'production' ? process.env.EMAIL_HOST_USER : process.env.MAILTRAP_USERNAME,
          pass: process.env.NODE_ENV === 'production' ? process.env.EMAIL_HOST_PASSWORD : process.env.MAILTRAP_PASSWORD
        }
    });
